...
class MultiHeadedSelfAttention_TDC(nn.Module):
    ...
    def forward(self, x, g_sharp):    # [B, 4*4*40, 128]
        ...
        scores = q @ k.transpose(-2, -1) / g_sharp
...
class Block_TDC(nn.Module):
    """Transformer Block"""
    def __init__(self, dim, num_heads, ff_dim, dropout, theta):
        super().__init__()
        self.attn = MultiHeadedSelfAttention_TDC(dim, num_heads, dropout, theta)
    ...
    def forward(self, x, g_sharp):
        Atten, Score = self.attn(self.norm1(x), g_sharp)
...
class Transformer_TDC(nn.Module):
    def __init__(self, num_layers, dim, num_heads, ff_dim, dropout, theta):
        super().__init__()
        self.blocks = nn.ModuleList([
            Block_TDC(dim, num_heads, ff_dim, dropout, theta) for _ in range(num_layers)])

    def forward(self, x, g_sharp):
        for block in self.blocks:
            x, Score = block(x, g_sharp)
        return x, Score

class TDC_ST(nn.Module):
    ...
    def __init__(
        ...
        # Transformer
        self.transformer1 = Transformer_TDC(num_layers=num_layers//3, dim=dim, num_heads=num_heads, 
                                       ff_dim=ff_dim, dropout=dropout_rate, theta=theta)
        # Transformer
        self.transformer2 = Transformer_TDC(num_layers=num_layers//3, dim=dim, num_heads=num_heads, 
                                       ff_dim=ff_dim, dropout=dropout_rate, theta=theta)
        # Transformer
        self.transformer3 = Transformer_TDC(num_layers=num_layers//3, dim=dim, num_heads=num_heads, 
                                       ff_dim=ff_dim, dropout=dropout_rate, theta=theta)
    ...
    def forward(self, x, g_sharp):
        ...
        Trans_features, Score1 =  self.transformer1(x, g_sharp)  # [B, 4*4*40, 64]
        Trans_features2, Score2 =  self.transformer2(Trans_features, g_sharp)  # [B, 4*4*40, 64]
        Trans_features3, Score3 =  self.transformer3(Trans_features2, g_sharp)  # [B, 4*4*40, 64]
