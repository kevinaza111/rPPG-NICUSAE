...
class TorchLossComputer(object):
    ...
    def cross_entropy_power_spectrum_loss(inputs, target, Fs):
        ...
        bpm_range = torch.arange(30, 220, dtype=torch.float).cuda()
    ...
    def cross_entropy_power_spectrum_DLDL_softmax2(inputs, target, Fs, std):
        ...
        target_distribution = [normal_sampling(int(target), i, std) for i in range(30, 220)]
        ...
        # target_distribution = torch.Tensor(target_distribution).to("cpu")
        ...
        # return loss_distribution_kl, F.cross_entropy(ca, (target - bpm_range[0]).view(1).type(torch.long)), math.fabs(
